 ...
 *
 * 1. one shell alone: `1 running shell · <command> · <Ns> · /shell to open` while the log fixture
 *    streams, then `1 shell · completed in <Ns> · /shell to open`;
 * 2. a long `sleep` next to the failing fixture, with the completed log shell still in the list:
 *    `3 shells · 2 running · 1 completed · /shell to open`, then once the failure landed
...
export default function (pi: ExtensionAPI): void {
    ...
    const logStreamCommand = getFixture("log-stream").command;
    ...
    faux.setResponses([
        // Turn 1: exactly one shell, so the summary uses its dedicated running/completed format.
        fauxAssistantMessage(
            fauxToolCall("bash", { command: logStreamCommand }),
            { stopReason: "toolUse" },
        ),
        // Turn 2: a long shell next to a failing one; the completed log shell keeps the list mixed,
