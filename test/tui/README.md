...
- `subagent-scenario.ts` (`subagent`) registers the same faux provider plus, at `session_start`, the
  offline probe agent `lune-shell-inspector-probe` (through `pi-subagents:runtime-agent-register:v1`). One
...
- `launch.ts` starts `pi` with the scenario's extensions loaded and stdio inherited, so the TUI draws
  in your terminal (or the pty you capture); it forwards the `LUNE_SHELL_INSPECTOR_*` variables and the
...
LUNE_SHELL_INSPECTOR_FIXTURE=log-stream npm run tui:demo
LUNE_SHELL_INSPECTOR_TIMEOUT=5 npm run tui:demo -- flood
LUNE_SHELL_INSPECTOR_COMMAND="sleep 30" LUNE_SHELL_INSPECTOR_PROBE_COMMAND="sleep 25" npm run tui:demo -- subagent
```

- Selection: positional argument first, then `LUNE_SHELL_INSPECTOR_SCENARIO`; anything that is not a
  scenario id is a fixture id. With nothing selected at all the launcher runs the `selection`
  scenario; a positional fixture id or `LUNE_SHELL_INSPECTOR_FIXTURE` (default `progress`) runs the
  single-call fixture scenario, where the fixture variables apply.
- `LUNE_SHELL_INSPECTOR_COMMAND` replaces the command outright; in the subagent scenario it replaces the
  parent's (default `sleep 30`, run in the background) and `LUNE_SHELL_INSPECTOR_PROBE_COMMAND` the probe's
  (default `sleep 25`, foreground). `LUNE_SHELL_INSPECTOR_TIMEOUT` becomes the bash tool's timeout in
  seconds. `LUNE_SHELL_INSPECTOR_MODE` is `foreground` (default) or `background` in the fixture scenario.
...
1. `$ <command>` - the tool row, drawn by pi's built-in bash renderer (bold prompt and command, plus
   a muted `(timeout Ns)` suffix when `LUNE_SHELL_INSPECTOR_TIMEOUT` set a timeout).
...
3. the settled row - the fixture's output, a truncation warning carrying the full output path for
   ...
   (`LUNE_SHELL_INSPECTOR_TIMEOUT=1 npm run tui:demo -- progress` kills the two-second fixture after one
...
With `LUNE_SHELL_INSPECTOR_MODE=background` the same fixture becomes a managed shell job:
...
2. `1 running shell · <command> · <Ns> · /shell to open` - the shell dock below the editor, mounted
   under widget key `lune-shell-inspector` with `placement: "belowEditor"`, with the seconds ticking while
   the command runs.
...
```
   1 running shell · sleep 30 · 0s · /shell to open   ← lune-shell-inspector (this extension)
...
(sleep 20) | timeout 35 script -q /tmp/pisv-tui-background.log env LUNE_SHELL_INSPECTOR_MODE=background npm run tui:demo -- progress >/dev/null 2>&1
...
(sleep 45) | timeout 60 script -q /tmp/pisv-tui-subagent.log \
  env LUNE_SHELL_INSPECTOR_COMMAND="sleep 30" LUNE_SHELL_INSPECTOR_PROBE_COMMAND="sleep 25" \
